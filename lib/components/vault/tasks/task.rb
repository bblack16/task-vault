...
module TaskVault
  class Task < SubComponent
    ...
    attr_element_of STATES, :status, default: :created, serialize: false
    attr_of [String, Integer, TrueClass, FalseClass, Range], :repeat, serialize: true, always: true
    attr_int_between 1, nil, :elevate_interval, default: nil, allow_nil: true, serialize: true, always: true
    attr_reader :run_count, :initial_priority, :timer, :times, serialize: false

    after :priority=, :set_initial_priority
    after :status=, :status_change
    after :delay=, :repeat=, :calculate_start_time
    ...
    def details(*attributes)
      attributes.map do |att|
        [att, (respond_to?(att.to_sym) ? send(att.to_sym) : nil)]
      end.to_h
    end
    ...
    def calculate_start_time
      ...
      elsif repeat.is_a?(Range)
        self.start_at = Time.now + rand(repeat)
      end
      rpt
    end
